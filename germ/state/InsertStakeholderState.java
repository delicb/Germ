...
public class InsertStakeholderState extends State {

	/**
	 * Dodaje Učesnika na poziciju kursora ako na toj poziciji već
	 * ne postoji nod.
	 */
    @Override
	public void mousePressed(MouseEvent e) {
		...
		if (e.getButton() == MouseEvent.BUTTON1) {
			...
			if (node == -1) {
				Node n = Stakeholder.createDefault(v.lastPosition, m.getNodeCount()+1);
				AddCommand command = new AddCommand(n);
				Application.getInstance().getCommandManager().doCommand(command);
			}
		}
	}
	...
	public void enter() {
		Application.getInstance().getView().setCursor(
				Cursors.getCursor("insertStakeholder"));
	}
}
